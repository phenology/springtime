...
def pycaret_ready(n=100):
    ...
    return pd.concat([combined, features], axis=1).drop('temperature', axis=1).rename(columns=dict(mean='mean temperature', max='maximum temperature', min='minimum temperature'))
